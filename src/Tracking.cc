...
void Tracking::setMasker(DynamicObjectMasker* masker){
    mpMasker = masker;
}
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    cv::Mat InputImage;
if (imRectLeft.channels() == 1)
    cv::cvtColor(imRectLeft, InputImage, cv::COLOR_GRAY2BGR);
else
    InputImage = imRectLeft.clone();
    
    mpMasker->setFrame(InputImage);
    torch::Tensor preprocessed = mpMasker->preprocess();
    torch::Tensor output = mpMasker->predict();
    mpMasker->postprocess(output);
    
    
    mpORBextractorLeft->mvDynamicObjectMask = mpMasker->getDynamicArea();
    //cout << "Tracking: mpORBextractorLeft->mvDynamicObjectMask size: " << (mpORBextractorLeft->mvDynamicObjectMask).size() << endl;
    
    
    {
         std::unique_lock<std::mutex> lock(mpViewer->mMutex);
         mpViewer->mvDynamicObject = mpMasker->getDynamicArea();
    }
    
    mpMasker->clearDynamic();
