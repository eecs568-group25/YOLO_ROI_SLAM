...
void Viewer::Run()
{
    ...
    pangolin::CreateWindowAndBind("YOLO-SLAM: Map Viewer",1024,768);
    ...
    Ow.SetIdentity();
    cv::namedWindow("YOLO-SLAM: Current Frame");
        ...
        // YOLO 动态物体检测
	{
            ...
            //cout << "Viewer: mvDynamicObject Size: " << mvDynamicObject.size() << endl;
           for (auto vit = mvDynamicObject.begin(); 	vit != mvDynamicObject.end(); vit++){
                cv::rectangle(toShow, *vit, cv::Scalar(0, 0, 255), 1);
            }
        }

        cv::imshow("YOLO-SLAM: Current Frame",toShow);
        cv::waitKey(mT);
